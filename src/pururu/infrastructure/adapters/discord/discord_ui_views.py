...
class SessionInfoLayoutView(discord.ui.LayoutView):
    ...
    def _build(self):
        accessory = discord.ui.Thumbnail(
            media=self.thumbnail_url) if not self.session.is_concluded() else discord_components.EditAttendanceButton(
            modal=EditAttendanceModal(session=self.session, callback=self.on_attendance_edit))
...
class EditAttendanceModal(discord.ui.Modal, title="Editar Asistencias"):
    ...
    def _build(self):
        ...
        self.add_item(
            discord_components.UserSelectorLabel("Usuarios Ausentes", "no editable, solo para tener una referencia",
                                                 absent_players_ids))
        players_with_justified_absence = [player.player_id for player in absent_players if player.justified_absence]
        self.justify_user_selector = discord_components.UserSelectorLabel("Justificar para:",
                                                                          "a que usuario de los anteriores se le va a justificar la ausencia",
        ...
        for i in range(min(len(absent_players), 3)):
            ...
            self.user_motive_dict[absent_player.player_id] = discord_components.MotiveTextInput(player_name,
                                                                                                absent_player.player_id,
                                                                                                absent_player.motive)
            self.add_item(self.user_motive_dict[absent_player.player_id])

    async def on_submit(self, interaction: discord.Interaction):
        justification_dict = {}
        motives_dict = {}
        justified_ids = [str(member.id) for member in self.justify_user_selector.component.values]
