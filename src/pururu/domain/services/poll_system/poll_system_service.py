...
class PollSystemService:
    def __init__(self, poll_repository, discord_service: DiscordService, poll_resolution_factory: PollResolutionFactory):
    ...
    def get_expired_polls(self) -> list[PollReference]:
        ...
        """
        polls: list[PollReference] = self.poll_repository.get_expired_polls()
    ...
    async def finalize_poll(self, poll_id: str) -> None:
        ...
        if not expired_poll:
            ...
            self.poll_repository.remove_poll(poll.id)
            return
